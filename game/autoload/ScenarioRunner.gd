...
func _step_actions() -> void:
	...
	if t == "host_multiplayer":
		var port: int = action.get("port", 7777)
	...
	if t == "join_multiplayer":
		var host: String = action.get("host", "127.0.0.1")
		var port: int = action.get("port", 7777)
	...
	if t == "simulate_player_join":
		var player_id: String = action.get("player_id", "test_player")
	...
	if t == "set_event_timestamp":
		var timestamp: int = action.get("timestamp", 0)
	...
	# ── Merchandise Actions ───────────────────────────────────────────
	if t == "open_shop":
		var category: String = action.get("category", "all")
	...
	if t == "open_promo":
		var promo_id: String = action.get("promo_id", "")
	...
	if t == "create_shareable_image":
		var include_branding: bool = action.get("include_branding", true)
	...
	if t == "cleanup_shares":
		var max_age: int = action.get("max_age", 86400)
