...
func _validate_scene(scene_path: String) -> void:
	...
	if not _require_keys(assets, ["ground", "walkmask_raw", "walkmask_player", "navpoly"], scene_path):
		return
	...
	var walkmask: Image = Image.new()
	var mask_error: Error = walkmask.load(player_path)
	if mask_error != OK:
		push_error("[Validator] Failed to load walkmask_player: %s (%s)" % [player_path, mask_error])
		_had_failure = true
		return
	...
	var ground_image: Image = Image.new()
	var ground_error: Error = ground_image.load(ground_path)
	if ground_error == OK and ground_image.get_size() != size_px:
	...
	for decal_data in decals:
		if typeof(decal_data) != TYPE_DICTIONARY:
			...
			_had_failure = true
			continue
		if not _require_keys(decal_data, ["id", "texture", "pos", "size"], scene_path):
			continue
